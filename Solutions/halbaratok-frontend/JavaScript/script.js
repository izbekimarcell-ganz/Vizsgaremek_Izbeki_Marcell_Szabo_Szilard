...
const APP_CONFIG = {
  apiBaseUrl:
    window.HalBaratokConfig?.apiBaseUrl || "http://localhost:4000/api",
  apiOrigin:
    window.HalBaratokConfig?.apiOrigin || "http://localhost:4000",
};
...
async function handleDeleteProfile() {
  ...
  try {
    const response = await fetch(`${APP_CONFIG.apiBaseUrl}/profile/delete`, {
      method: "DELETE",
      headers: {
...
window.HalBaratokShared = {
  ...
  apiOrigin: APP_CONFIG.apiOrigin,
  apiRequest,
  getAuthHeaders,
  getStoredUser,
  showAppAlert,
  ...
  isLoggedIn,
  setPendingRedirect,
  formatForumReportReason(reasonCode) {
